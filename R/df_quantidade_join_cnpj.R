...
df_quantidade_cnpj <-
  ...
  dplyr::filter(enquadramentoJuridico == "CNPJ") %>%
...
# Base final Arrec
base_quantidade_final <- df_quantidade_cnpj_join_nat_jur_cnae %>%
  dplyr::select(-c('indicador', 'tipo_de_registro', 'tipo_atualizacao',
...
write_csv(base_quantidade_final, "data-raw/base_quantidade_final.csv")

# usethis::use_data(base_quantidade_final, overwrite = TRUE)
